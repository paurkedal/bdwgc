...
/* This is a variant of disclaim_test.c without the disclam notifiers, and
 * serves as a stress-test for multithreaded allocation. */
...
pair_new(pair_t car, pair_t cdr)
{
    ...
    p = GC_malloc(sizeof(struct pair_s));
    if (p == NULL) {
        fprintf(stderr, "Out of memory!\n");
        exit(3);
    }
